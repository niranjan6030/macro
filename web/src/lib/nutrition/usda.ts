...
export async function search(query: string, limit = 15): Promise<Food[]> {
  ...
  try {
    ...
    return foods
      .map((f) => toFood(f as Record<string, unknown>))
...
export async function byId(fdcId: string): Promise<Food | null> {
  if (!KEY) return null;
  try {
...
function toFood(f: Record<string, unknown>): Food | null {
  const name = str(f.description);
  if (!name) return null;

  const values = new Map<number, number>();
  const list = (f.foodNutrients ?? []) as Record<string, unknown>[];
  for (const item of list) {
    // The search endpoint and the detail endpoint nest this differently.
    const id = Number(
      item.nutrientId ?? (item.nutrient as Record<string, unknown>)?.id ?? NaN,
    );
