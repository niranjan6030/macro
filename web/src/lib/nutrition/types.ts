...
export type FoodSource = "openfoodfacts" | "usda" | "custom" | "estimate";

export interface Food {
  /** Stable within a source: barcode for OFF, fdcId for USDA. */
  id: string;
  source: FoodSource;
  name: string;
  brand?: string | null;
  /** Per 100 g. Always. */
  per100g: Nutrients;
  /** What the packet calls one serving, in grams. Convenience only. */
  servingG?: number | null;
  servingLabel?: string | null;
  barcode?: string | null;
  /**
   * How much to trust these numbers.
   *   "label"    — read off a manufacturer's declared panel
   *   "measured" — laboratory analysis (USDA Foundation / SR Legacy)
   *   "estimated" — inferred, e.g. a cooked dish broken into components
   */
  confidence: "label" | "measured" | "estimated";
}

export const EMPTY: Nutrients = {
...
export const sum = (list: Nutrients[]): Nutrients => list.reduce(add, EMPTY);
 ...
 */
export function plausible(n: Nutrients): boolean {
...
const r = (n: number) => Math.round((Number.isFinite(n) ? n : 0) * 10) / 10;
