...
export async function searchAll(query: string, limit = 20): Promise<Food[]> {
  const q = query.trim();
  ...
  const [usdaHits, offHits] = await Promise.all([
    usda.search(q, 12).catch(() => [] as Food[]),
    off.search(q, 12).catch(() => [] as Food[]),
  ]);
 ...
 */
function score(f: Food, q: string): number {
...
function dedupe(list: Food[]): Food[] {
  ...
  const rank = { measured: 3, label: 2, estimated: 1 } as const;

  for (const f of list) {
    const key = f.name.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 24)
      + (f.brand ? `|${f.brand.toLowerCase()}` : "");
...
export async function resolve(source: string, id: string): Promise<Food | null> {
  switch (source) {
    case "custom": return indian.byId(id);
    case "usda": return usda.byId(id);
    case "openfoodfacts": return off.byBarcode(id);
    default: return null;
  }
}
