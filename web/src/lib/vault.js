...
export async function open(key, iv, cipher) {
...
export async function makeVerifier(key) {
  const { iv, cipher } = await seal(key, enc.encode(VERIFIER));
  return `${iv}.${cipher}`;
}
...
export const rememberKey = (uid, key) => withStore("readwrite", (s) => s.put(key, uid));

export const recallKey = (uid) => withStore("readonly", (s) => s.get(uid));

export const forgetKey = (uid) => withStore("readwrite", (s) => s.delete(uid));
