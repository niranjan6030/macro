import "server-only";
import { type Nutrients, plausible } from "@/lib/nutrition/types";
...
const TOOL: ToolSpec = {
  name: "record_recipe",
  description:
    "Break the dish into its ingredients with cooked weights. Call this exactly once.",
  parameters: {
    type: "object",
    properties: {
      serves: {
      ...
      ingredients: {
        type: "array",
        description: "Every ingredient with a meaningful calorie contribution. Include cooking oil and ghee — they are usually the largest single item and the easiest to forget.",
        items: {
          type: "object",
          properties: {
            search_query: {
              type: "string",
              description: "Plain generic name for a nutrition database: 'toor dal cooked', 'sunflower oil', 'onion raw'.",
            },
            grams: {
              type: "number",
              description: "Total cooked weight of this ingredient across the whole batch, in grams.",
            },
          },
      ...
      note: {
        type: "string",
        description: "Anything that would change the numbers a lot and is worth telling the person.",
      },
    },
...
export interface Estimate {
  per100g: Nutrients;
  /** What it was built from, so the person can check the working. */
  ingredients: { name: string; grams: number; kcal: number; matched: boolean }[];
  serves: number;
  totalGrams: number;
  note?: string;
  /** Set when the estimate could not be made. */
  error?: string;
}

export async function estimate(dish: string, description?: string): Promise<Estimate> {
  const empty: Nutrients = {
    kcal: 0, protein: 0, carbs: 0, fat: 0, fibre: 0, sugar: 0, satFat: 0, sodium: 0,
  };

  if (!aiConfigured()) {
    return {
      per100g: empty, ingredients: [], serves: 1, totalGrams: 0,
      error: "Estimating needs an AI key — ANTHROPIC_API_KEY, or GEMINI_API_KEY for "
        + "Google's free tier. Until then, enter the panel by hand.",
    };
  }

  let recipe: { serves?: number; ingredients?: { search_query: string; grams: number }[]; note?: string };
  try {
    ...
    recipe = call.args as typeof recipe;
  } catch (e) {
  ...
  if (totalGrams <= 0) {
    return {
      per100g: empty, ingredients, serves: recipe.serves ?? 1, totalGrams: 0,
  ...
  if (!plausible(per100g)) {
    return {
      per100g: empty, ingredients, serves: recipe.serves ?? 1, totalGrams: Math.round(totalGrams),
      error: "The numbers that came back did not add up, so they are not being offered. "
