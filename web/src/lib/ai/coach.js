...
export function analyse(data) {
  ...
  let trendChangeKg = null;
  if (weights.length >= 4) {
    trendChangeKg = round1(weights.at(-1).trendKg - weights[0].trendKg);
  }

  let predictedChangeKg = null;
  let impliedTdee = null;
 ...
 */
function fallback(f, profile) {
  const parts = [];

  if (f.daysLogged === 0) {
    return (
      "Nothing logged in this window yet. Log a few days and this becomes useful — " +
  ...
  if (f.trendChangeKg != null) {
    ...
    parts.push(
      `Your trend weight is ${dir}${f.trendChangeKg !== 0 ? ` ${Math.abs(f.trendChangeKg)} kg` : ""}.`,
    );
  }
  ...
  if (parts.length <= 2) {
    parts.push(
      `Keep going — ${profile.goal === "lose" ? "the deficit only works if it is boring" : "consistency is the whole game"}.`,
    );
  }
  return parts.join(" ");
}
...
const daySpan = (a, b) => Math.max(1, Math.round((Date.parse(b) - Date.parse(a)) / 86_400_000));
