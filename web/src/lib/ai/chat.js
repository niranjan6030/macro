  ...
  parameters: {
    ...
    properties: {
      query: {
        type: "string",
        description:
          "Plain generic name and cooking method: 'roti', 'toor dal cooked', 'chicken breast grilled'.",
      },
      grams: {
        type: "number",
...
export async function reply(history, context) {
  if (!aiConfigured()) {
    return {
      text:
  ...
  for (let round = 0; round < 6; round++) {
    ...
    toolResults = await Promise.all(
      res.calls.map(async (call) => {
        const { query, grams } = call.args;
  ...
  return {
    text:
      "That needed more looking up than I can do in one go. Try asking " +
...
async function lookUp(query, grams) {
