 ...
 */
const GEMINI_FALLBACKS = ["gemini-3.6-flash", "gemini-3.5-flash"];
...
function isOverloaded(e) {
  ...
  return /UNAVAILABLE|RESOURCE_EXHAUSTED|DEADLINE_EXCEEDED|high demand|overloaded|timed? ?out/i.test(
    text,
  );
}

export async function runRound(round) {
  switch (provider()) {
    case "anthropic":
      return anthropicRound(round);
    case "gemini":
      return geminiRound(round);
    default:
...
async function anthropicRound(r) {
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

  const messages = r.history.map((t, i) => {
    ...
    if (i === 0 && r.image && t.role === "user") {
      return {
        ...
        content: [
          {
            type: "image",
            source: { type: "base64", media_type: r.image.mediaType, data: r.image.base64 },
          },
  ...
  const res = await client.messages.create({
    ...
    ...(r.tools?.length
      ? {
          ...
          ...(r.forceTool ? { tool_choice: { type: "tool", name: r.forceTool } } : {}),
        }
      : {}),
    messages,
  });
...
async function geminiRound(r) {
  ...
  const contents = r.history.map((t, i) => {
    const parts = [];
    if (i === 0 && r.image && t.role === "user") {
