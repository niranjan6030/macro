...
export type ActivityKind = "cardio" | "hiit" | "sport" | "class" | "daily";

export interface Activity {
  id: string;
  name: string;
  kind: ActivityKind;
  /** Metabolic equivalent, gross. */
  met: number;
  /** Shown when the intensity is the thing that matters. */
  detail?: string;
}
...
export const LIFTING_MET = { light: 3.5, moderate: 5.0, hard: 6.0 } as const;
...
export const byId = (id: string): Activity | null =>
  ACTIVITIES.find((a) => a.id === id) ?? null;

export const byKind = (kind: ActivityKind): Activity[] =>
