...
import {
  type Profile, bmr, estimateBodyFatPct, round1, tdee,
} from "./energy";
...
export interface Projection {
  /** Week-by-week, capped at two years. */
  weeks: ProjectionWeek[];
  /** Weeks to reach the target, or null if it is not reachable at this intake. */
  weeksToGoal: number | null;
  /** Calendar date of arrival, ISO, or null. */
  goalDate: string | null;
  startWeightKg: number;
  targetWeightKg: number | null;
  /** What the projection says about the plan, in one sentence. */
  verdict: string;
}
 ...
 */
export function project(profile: Profile, intakeKcal: number, from = new Date()): Projection {
  ...
  for (let week = 1; week <= MAX_WEEKS; week++) {
    ...
    const adaptation = inDeficit
      ? MAX_ADAPTATION * (1 - Math.exp(-week / ADAPTATION_WEEKS))
      : 0;
  ...
  const goalDate = weeksToGoal != null
 ...
 */
export function intakeForDeadline(profile: Profile, weeks: number): number | null {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (!at) break;
    if (at.weightKg > target) hi = mid; else lo = mid;
  }
