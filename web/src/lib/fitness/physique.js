...
export const BUILDS = {
  underweight: {
    ...
    label: "Underweight",
    meaning:
      "BMI under 18.5. Low fat and low muscle both — there is not much tissue of either kind.",
 ...
 */
export function classify(bmi, bodyFatPct, ffmi, sex) {
  const male = sex === "male";
...
export function physiqueOf(c) {
  ...
  const w = {
    neck: 0.03 + muscularity * 0.008 + adiposity * 0.006,
    ...
    hip: (male ? 0.09 : 0.098) + muscularity * 0.005 + adiposity * 0.019,
    thigh: (male ? 0.044 : 0.047) + muscularity * 0.01 + adiposity * 0.011,
  ...
  const centralBias = clamp(
    (male ? 0.52 : 0.24) + (1 - muscularity) * 0.42 - adiposity * 0.2,
    0,
    1,
  );
