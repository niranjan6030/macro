...
import { byKind, caloriesBurned, grossCalories } from "@/lib/fitness/activities";
import {
  EQUIPMENT_LABEL,
  MUSCLE_LABEL,
  SPLITS,
...
export function TrainPicker({ weightKg, onPickExercise, onLogActivity, onClose }) {
  const [stage, setStage] = useState({ at: "browse" });
  const [picked, setPicked] = useState(new Set(["gym"]));
                ...
                <li key={kind}>
                  ...
                  >
                    {on && (
                      <Check
                        size={14}
                        className="absolute right-3 top-3 text-[var(--color-chalk)]"
                      />
                    )}
                    <Icon
                      size={19}
                      className={on ? "text-[var(--color-chalk)]" : "text-[var(--color-mute)]"}
                    />
                ...
                <>
                  ...
                  <div>
                    ...
                    <ul className="flex flex-wrap gap-2">
                      {Object.keys(MUSCLE_LABEL).map((m) => (
                        <li key={m}>
                          <button
                            onClick={() => setStage({ at: "muscle", muscle: m })}
                            className="chip"
                          >
                            {MUSCLE_LABEL[m]}
                          </button>
            ...
            <section key={kind} className="border-t border-[var(--color-line)] pt-4">
              ...
              <ul className="space-y-2">
                {byKind(kind).map((a) => (
                  <li key={a.id}>
                    <button
                      onClick={() => setStage({ at: "duration", activity: a })}
                ...
                {mine.map((e) => (
                  <li key={e.id} className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span className="block text-[11px] text-[var(--color-mute)]">
                        {MUSCLE_LABEL[e.primary]} · {EQUIPMENT_LABEL[e.equipment]} · {e.repRange[0]}
                        –{e.repRange[1]} reps
                      </span>
                    </button>
                    <button
    ...
    <li>
      ...
      >
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-[11px] leading-relaxed text-[var(--color-mute)]">
            {def.blurb}
          </span>
        </span>
        <ChevronDown
        ...
        <div className="mt-2 space-y-3 border-l border-[var(--color-line)] pl-3">
          {def.sessions.map((session) => (
            <section key={session.name}>
              ...
              <ExerciseList
                list={session.exerciseIds.map(byId).filter((e) => e !== null)}
                onPick={onPick}
              />
            </section>
          ))}
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[11px] leading-relaxed text-[var(--color-mute)]">
          It joins the library for you alone, and progresses the same way — the weight goes up once
          you hit the top of the rep range on every set.
        </p>
      </div>
      ...
      <div className="card space-y-3 p-4">
        ...
        <div>
          <label className="label" htmlFor="ex-kit">
            Equipment
          </label>
          <select
            id="ex-kit"
            ...
            className="field"
          >
            {Object.keys(EQUIPMENT_LABEL).map((k) => (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="label" htmlFor="ex-low">
              Reps from
            </label>
            <input
              id="ex-low"
          ...
          <div>
            <label className="label" htmlFor="ex-high">
              Reps to
            </label>
            <input
              id="ex-high"
        ...
        <div>
          <label className="label" htmlFor="ex-note">
            Cue <span className="normal-case">(optional)</span>
          </label>
          <input
            id="ex-note"
            value={note}
            ...
            className="field"
            placeholder="Arms straight, squeeze the shoulder blades"
            maxLength={200}
          />
        </div>
      </div>
...
function ExerciseList({ list, onPick }) {
  if (!list.length) {
    return (
    ...
    <div className="space-y-4">
      ...
      <div className="card p-4">
        ...
        <div className="mt-1 flex flex-wrap gap-1.5">
          {[15, 30, 45, 60, 90].map((m) => (
            <button key={m} onClick={() => setMinutes(m)} className="chip">
              {m} min
            </button>
          ))}
        </div>
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-[var(--color-mute)]">
          ...
          <span className="num">{gross}</span> — that figure includes your resting burn, which your
          maintenance already counts, so adding it to a day counts it twice.
        </p>
      </div>
      ...
      <button
        ...
        onClick={async () => {
          setBusy(true);
          try {
            await onLog(activity, minutes, net);
          } finally {
            setBusy(false);
          }
        }}
