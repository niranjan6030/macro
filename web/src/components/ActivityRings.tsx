...
export interface Ring {
  ...
  value: number;
  target: number;
  /** What the number means: "kcal", "g", "sets". */
  unit: string;
...
export function ActivityRings({
  rings, size = 196, stroke = 15, gap = 5, children,
}: {
  rings: Ring[];
  size?: number;
  stroke?: number;
          ...
          return (
            <g key={ring.label}>
              <circle
                ...
                stroke={ring.tone} strokeOpacity={0.16} strokeWidth={stroke}
              />
              <circle
                cx={size / 2} cy={size / 2} r={r} fill="none"
                stroke={ring.tone} strokeWidth={stroke} strokeLinecap="round"
                strokeDasharray={c}
                strokeDashoffset={c * (1 - lap1)}
                filter={closed ? "url(#ring-glow)" : undefined}
                style={{
                  transition: reduced ? undefined : `stroke-dashoffset 1100ms cubic-bezier(.15,.75,.25,1) ${i * 110}ms`,
                }}
              />
              {lap2 > 0 && (
                <circle
                  cx={size / 2} cy={size / 2} r={r} fill="none"
                  stroke="#000" strokeOpacity={0.55} strokeWidth={stroke * 0.42}
                  strokeLinecap="round"
                  strokeDasharray={c}
                  strokeDashoffset={c * (1 - lap2)}
                  style={{
                    transition: reduced ? undefined : `stroke-dashoffset 1100ms cubic-bezier(.15,.75,.25,1) ${i * 110 + 300}ms`,
                  }}
                />
              )}
            </g>
...
export function RingKey({ rings }: { rings: Ring[] }) {
  return (
        ...
        return (
          <li key={r.label} className="flex items-baseline gap-2.5">
            ...
            <span className="num ml-auto text-sm">
              ...
              <span className="text-[var(--color-mute)]">/{r.target} {r.unit}</span>
            </span>
          </li>
        );
      })}
