...
import {
  checkVerifier, deriveKey, makeVerifier, open, randomSalt, seal as sealBytes,
  recallKey, rememberKey,
} from "@/lib/vault";
...
interface VaultValue {
  /** null while loading, false when not set up yet, true when unlocked. */
  state: "loading" | "absent" | "locked" | "open";
  retentionDays: number;
  setUp: (passphrase: string, days: number) => Promise<string | null>;
  unlock: (passphrase: string) => Promise<string | null>;
  seal: (bytes: Uint8Array<ArrayBuffer>) => Promise<{ iv: string; cipher: string }>;
  reveal: (url: string, iv: string | null) => Promise<string | null>;
  setRetention: (days: number) => Promise<void>;
}

const Ctx = createContext<VaultValue | null>(null);
...
export function VaultProvider({ children }: { children: React.ReactNode }) {
  ...
  const [state, setState] = useState<VaultValue["state"]>("loading");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!vault) { setState("absent"); return; }
        setSalt(vault.salt); setVerifier(vault.verifier); setDays(vault.retention_days);
  ...
  const setUp = useCallback(async (passphrase: string, days: number) => {
    const s = randomSalt();
    ...
    setSalt(s); setVerifier(v); setDays(days); setKey(k); setState("open");
    return null;
  }, [uid]);
  ...
  const unlock = useCallback(async (passphrase: string) => {
    ...
    await rememberKey(uid, k);
    setKey(k); setState("open");
    return null;
  }, [salt, verifier, uid]);

  const seal = useCallback(async (bytes: Uint8Array<ArrayBuffer>) => {
   ...
   */
  const reveal = useCallback(async (url: string, iv: string | null) => {
    if (!iv) return url;
    if (!key) return null;
    try {
      const bytes = new Uint8Array(await (await fetch(url)).arrayBuffer());
      const plain = await open(key, iv, bytes as Uint8Array<ArrayBuffer>);
...
export function VaultGate() {
  ...
  const go = async () => {
    setError(null);
    if (first) {
      if (pass.length < 10) { setError("Use at least ten characters."); return; }
      if (pass !== again) { setError("Those two do not match."); return; }
    }
    setBusy(true);
    try {
  ...
  return (
    <div className="card space-y-4 p-5">
      ...
      <div className="space-y-2.5">
        <div>
          <label className="label" htmlFor="vault-pass">Passphrase</label>
          <input
            id="vault-pass" type="password" value={pass} autoComplete={first ? "new-password" : "current-password"}
            onChange={(e) => setPass(e.target.value)} className="field"
            placeholder={first ? "Something you will not forget" : ""}
            onKeyDown={(e) => { if (e.key === "Enter" && !first) go(); }}
          />
        </div>
        ...
        {first && (
          <>
            <div>
              <label className="label" htmlFor="vault-again">Type it again</label>
              <input id="vault-again" type="password" value={again} autoComplete="new-password"
            ...
            <div>
              ...
              <p className="mt-1.5 text-[11px] leading-relaxed text-[var(--color-mute)]">
                After that the image is deleted automatically. The date, the pose and your
                weight that day are kept — that is what the chart is drawn from, and it
                costs a few bytes instead of a few megabytes. You can change this later.
              </p>
            </div>
          </>
        )}
      ...
      {first && (
        <p className="flex gap-1.5 rounded-lg bg-[var(--color-slab-2)] p-3 text-[11px] leading-relaxed text-[var(--color-mute)]">
          ...
          There is no way to reset this. If you forget the passphrase the photos cannot be
          recovered — not by you and not by us. That is the trade for nobody else being
          able to open them.
        </p>
      )}
      ...
      <button disabled={busy || !pass} onClick={go} className="btn btn-primary w-full">
        {busy ? <Loader2 className="animate-spin" size={16} />
              : first ? <Lock size={16} /> : <Check size={16} />}
...
export function SealedImage({ url, iv, alt, className }: {
  url: string | null; iv: string | null; alt: string; className?: string;
}) {
  ...
  useEffect(() => {
    ...
    reveal(url, iv).then((u) => {
      if (!live) { if (u?.startsWith("blob:")) URL.revokeObjectURL(u); return; }
