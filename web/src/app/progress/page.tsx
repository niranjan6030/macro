...
export default function ProgressPage() {
  const [tab, setTab] = useState<"photos" | "tape">("photos");
  ...
  return (
    <div className="space-y-4 py-6">
      ...
      <div className="flex gap-1 rounded-xl bg-[var(--color-slab-2)] p-1">
        {([["photos", "Photos", Camera], ["tape", "Measurements", Ruler]] as const).map(([k, label, Icon]) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        disabled={busy}
        onClick={async () => {
          if (isNative()) { const p = await nativePhoto(); if (p) upload(p); }
          ...
          <ul className="grid grid-cols-3 gap-2">
            {list.map((p) => (
              <li key={p.id} className="relative">
                ...
                <button aria-label={`Delete photo from ${p.on_date}`}
                        onClick={async () => { await del(`/api/progress/photos?id=${p.id}`); reload(); }}
      ...
      {!list.length && !busy && (
        <p className="card p-6 text-center text-sm leading-relaxed text-[var(--color-mute)]">
          No {pose} photos yet. Take one now — same spot, same light, same time of day.
          In eight weeks it is the most convincing thing in this app.
        </p>
      )}
    </div>
  );
...
const FIELDS = [
  ["neck_cm", "Neck"], ["chest_cm", "Chest"], ["waist_cm", "Waist"],
...
function Tape() {
  ...
  const fetcher = useCallback(
    () => get<{ measurements: Measurement[] }>("/api/progress/measurements"),
    [],
  );
    ...
    <div className="space-y-4">
      <form
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            const res = await post<{ derived: boolean; measurement: Measurement }>(
              "/api/progress/measurements", { date: today(), ...payload },
            );
            if (res.derived && res.measurement.body_fat_pct) {
              setNote(`Body fat works out at about ${res.measurement.body_fat_pct}% from those measurements.`);
            }
            setDraft({}); reload();
          } finally { setBusy(false); }
        }}
      >
        <p className="text-xs leading-relaxed text-[var(--color-mute)]">
          Measure relaxed, first thing, same spot each time. Neck and waist alone are
          enough for Macro to work out your body fat — and that makes every calorie
          target more accurate, because expenditure tracks lean mass, not total weight.
        </p>
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {FIELDS.map(([k, label]) => (
            <div key={k}>
              ...
              <input id={k} inputMode="decimal" className="field num px-2 text-sm"
                     value={draft[k] ?? ""} placeholder="cm"
        ...
        <section className="card p-4">
          ...
          {latest.body_fat_pct && (
            <p className="num mt-3 border-t border-[var(--color-line)] pt-3 text-sm">
              Body fat <span className="font-bold text-[var(--color-volt)]">{latest.body_fat_pct}%</span>
