...
export default function TodayPage() {
  ...
  const fetcher = useCallback(
    () => get<DayResponse>(`/api/day?date=${date}`),
    [date],
  );
  ...
  async function patchDay(patch: Record<string, unknown>) {
    ...
    <div className="space-y-5 py-6">
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link href={`/food?date=${date}`} className="flex items-center gap-1 text-sm font-semibold text-[var(--color-volt)]">
            <Plus size={16} /> Add
          </Link>
        </div>
              ...
              return (
                <div key={meal}>
                  <div className="mb-1.5 flex justify-between text-xs font-semibold uppercase tracking-wide text-[var(--color-mute)]">
                    <span>{meal}</span><span className="num">{kcal} kcal</span>
                  </div>
                  <ul className="card divide-y divide-[var(--color-line)]">
                    {items.map((e) => (
                      <li key={e.id} className="flex items-center gap-3 p-3">
                        ...
                        <span className="num text-sm font-semibold">{Math.round(e.nutrients.kcal)}</span>
                        <button
                          aria-label={`Remove ${e.name}`}
                          onClick={async () => { await del(`/api/diary?id=${e.id}`); reload(); }}
      ...
      {data?.trend && data.trend.length > 1 && (
        <section className="card p-4">
          ...
          <p className="mt-2 text-xs leading-relaxed text-[var(--color-mute)]">
            Dots are what the scale said. The line is the seven-day average — that is
            the one that reflects fat, rather than yesterday&apos;s salt.
          </p>
        </section>
      )}
    </div>
...
function Toggle({ on, onClick, Icon, label, hint }: {
  ...
  return (
    <button onClick={onClick} aria-pressed={on}
      ...
      <span style={{ color: on ? "var(--color-volt)" : "var(--color-mute)" }}><Icon size={18} /></span>
...
function WeighIn({ value, onSave }: { value: number | null; onSave: (kg: number) => void }) {
  ...
  return (
    <form className="card flex gap-2 p-3"
          onSubmit={(e) => {
            ...
            if (kg >= 25 && kg <= 400) { onSave(kg); setEditing(false); }
          }}>
      <input autoFocus inputMode="decimal" className="field num flex-1" value={draft}
             onChange={(e) => setDraft(e.target.value)} placeholder="78.5" aria-label="Weight in kilograms" />
