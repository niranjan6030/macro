...
export default function CoachPage() {
  ...
  const fetcher = useCallback(
    () => get<CoachResponse>(`/api/coach?date=${today()}${refresh ? "&refresh=1" : ""}`),
    [refresh],
  );
            ...
            <section className="card p-4">
              ...
              {f.notes.length > 0 && (
                <ul className="mt-4 space-y-1.5 border-t border-[var(--color-line)] pt-3">
                  {f.notes.map((n, i) => (
                    <li key={i} className="text-xs leading-relaxed text-[var(--color-mute)]">· {n}</li>
                  ))}
                </ul>
              )}
            </section>
...
function Projection({ title, text, accent }: { title: string; text: string; accent?: boolean }) {
  return (
    <div className="rounded-xl border p-3"
         style={{
           borderColor: accent ? "var(--color-volt-dim)" : "var(--color-line)",
           background: accent ? "color-mix(in srgb, var(--color-volt) 6%, transparent)" : "var(--color-slab-2)",
         }}>
      <p className="text-[10px] font-semibold uppercase tracking-wide"
         style={{ color: accent ? "var(--color-volt)" : "var(--color-mute)" }}>{title}</p>
