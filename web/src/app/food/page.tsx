...
function PhotoTab({ date, onError, onSaved }: {
  ...
  async function run(dataUrl: string) {
    setImage(dataUrl); setResult(null); setBusy(true); onError("");
    try {
      setResult(await post<IdentifyResponse>("/api/food/identify", { image: dataUrl }));
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      {image && !busy && (
        <button onClick={() => { setImage(null); setResult(null); }} className="btn btn-ghost w-full">
          Take another
        </button>
      )}
...
function IdentifiedCard({ item, date, onError, onSaved }: {
  item: IdentifiedItemShape; date: string; onError: (m: string) => void; onSaved: () => void;
}) {
    ...
    return (
      <div className="card p-4">
        ...
        <p className="mt-1 text-sm text-[var(--color-mute)]">
          Recognised, but not in any nutrition database. Add it by hand and it will be
          there next time.
        </p>
      </div>
    );
  ...
  return (
    <div className="card p-4" style={{ opacity: done ? 0.55 : 1 }}>
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          inputMode="decimal" value={grams}
          onChange={(e) => setGrams(Math.max(0, Number(e.target.value) || 0))}
          className="field num w-24" aria-label={`Grams of ${item.label}`}
        />
        <span className="text-sm text-[var(--color-mute)]">grams</span>
        {item.food.servingG && (
          <button onClick={() => setGrams(item.food!.servingG!)}
      ...
      {shown && (
        <dl className="num mt-3 grid grid-cols-5 gap-1 border-t border-[var(--color-line)] pt-3 text-center text-xs">
          {([["kcal", shown.kcal], ["P", shown.protein], ["C", shown.carbs],
             ["F", shown.fat], ["Fib", shown.fibre]] as const).map(([l, v]) => (
            <div key={l}>
...
function Provenance({ food, confidence }: { food: Food; confidence?: string }) {
  const label =
    food.confidence === "label" ? "From the packet"
    : food.confidence === "measured" ? "Lab measured"
    : "Estimate";
  const colour =
    food.confidence === "measured" ? "var(--color-fibre)"
    : food.confidence === "label" ? "var(--color-protein)"
    : "var(--color-mute)";

  return (
    <span className="shrink-0 text-right">
      <span className="chip" style={{ color: colour, borderColor: colour }}>{label}</span>
      {confidence === "low" && (
...
function SearchTab({ date, onError, onSaved }: {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (digits.length >= 8 && digits.length === q.trim().length) {
          const { food } = await get<{ food: Food }>(`/api/food/barcode/${digits}`);
          setFoods([food]);
        } else {
          setFoods((await get<{ foods: Food[] }>(`/api/food/search?q=${encodeURIComponent(q)}`)).foods);
        }
      } catch (e) {
  ...
  if (chosen) {
    return <PortionPicker food={chosen} date={date} onError={onError}
                          onSaved={() => { onSaved(); setChosen(null); setQ(""); }}
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        <Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-mute)]" />
        <input autoFocus value={q} onChange={(e) => setQ(e.target.value)}
               className="field pl-10" placeholder="Dal, chicken breast, or a barcode"
               aria-label="Search for a food" />
        {busy && <Loader2 size={16} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-[var(--color-mute)]" />}
      </div>

      <p className="flex gap-1.5 px-1 text-xs text-[var(--color-mute)]">
        <ScanLine size={13} className="mt-px shrink-0" />
        Type the numbers under a barcode for the manufacturer&apos;s own panel — the most
        accurate entry there is.
      </p>

      <ul className="space-y-2">
        {foods.map((f) => (
          <li key={`${f.source}-${f.id}`}>
            <button onClick={() => setChosen(f)} className="card flex w-full items-center gap-3 p-3.5 text-left">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">{f.name}</p>
                <p className="num truncate text-xs text-[var(--color-mute)]">
                  {f.brand && `${f.brand} · `}{Math.round(f.per100g.kcal)} kcal / 100 g
                </p>
              </div>
              <Provenance food={f} />
            </button>
      ...
      {!busy && q.trim().length >= 2 && !foods.length && (
        <p className="card p-6 text-center text-sm text-[var(--color-mute)]">
          Nothing found. Try a simpler name — &ldquo;dal&rdquo; rather than
          &ldquo;mum&rsquo;s dal&rdquo; — or add it by hand.
        </p>
      )}
    </div>
  );
...
function PortionPicker({ food, date, onError, onSaved, onBack }: {
  food: Food; date: string;
  onError: (m: string) => void; onSaved: () => void; onBack: () => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-[var(--color-mute)]">
      ...
      <div className="card p-4">
        ...
        <div className="mt-3"><Provenance food={food} /></div>

        <label className="label mt-4">How much?</label>
        <div className="flex items-center gap-2">
          <input autoFocus inputMode="decimal" value={grams} className="field num flex-1"
                 onChange={(e) => setGrams(Math.max(0, Number(e.target.value) || 0))} />
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {[50, 100, 150, 200, 250].map((g) => (
            <button key={g} className="chip" onClick={() => setGrams(g)}>{g} g</button>
          ))}
        </div>
      ...
      <button
        disabled={busy || grams <= 0} className="btn btn-primary w-full"
        onClick={async () => {
          setBusy(true);
          try {
            await post("/api/diary", {
  ...
  return (
        ...
        try {
          await post("/api/diary", {
            ...
            per_100g: {
              ...
              fibre: Number(per.fibre) || 0, sugar: 0, satFat: 0, sodium: 0,
            },
          });
          await tapFeedback();
          onSaved();
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="label" htmlFor="mgrams">You ate (g)</label>
          <input id="mgrams" inputMode="decimal" required className="field num" value={grams}
...
function scale(per100g: Nutrients, grams: number): Nutrients {
  const f = grams / 100;
  return {
    kcal: per100g.kcal * f, protein: per100g.protein * f, carbs: per100g.carbs * f,
    fat: per100g.fat * f, fibre: per100g.fibre * f, sugar: per100g.sugar * f,
    satFat: per100g.satFat * f, sodium: per100g.sodium * f,
  };
}
