...
import { getProfile, upsertProfile, profileFor, latestWeight, type StoredProfile } from "@/lib/db";
...
export const PUT = withUser(async (uid, req) => {
  ...
  if ("units" in b) patch.units = oneOf(b.units, ["metric", "imperial"] as const) ?? "metric";
  ...
  for (const k of ["protein_override", "carbs_override", "fat_override"] as const) {
