...
export const GET = withUser(async (uid, req) => {
  ...
  if (!refresh) {
    ...
    if (cached) {
      return ok({
        ...(await context(uid, profile, since, today, window)),
        body: cached.body,
        cached: true,
      });
    }
  ...
  await db()
    .from("coach_notes")
    .upsert({ uid, on_date: today, kind: "weekly", body }, { onConflict: "uid,on_date,kind" })
...
async function context(uid, profile, since, today, window) {
