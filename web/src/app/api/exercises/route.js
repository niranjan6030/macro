...
import { MUSCLE_LABEL, EQUIPMENT_LABEL } from "@/lib/fitness/training";

const MUSCLES = Object.keys(MUSCLE_LABEL);
const EQUIPMENT = Object.keys(EQUIPMENT_LABEL);
...
export const POST = withUser(async (uid, req) => {
  ...
  const exercise = await addCustomExercise(uid, {
    ...
    equipment,
    rep_low: Math.round(low),
    rep_high: Math.round(high),
    note: str(b.note, 200),
  });
