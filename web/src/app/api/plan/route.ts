...
import {
  SPLITS, byId, nextPrescription, splitFor, weekPlan, weeklyVolume,
  type SplitName,
...
export const GET = withUser(async (uid, req) => {
  ...
  const split = ((profile?.split as SplitName) in SPLITS
    ? profile!.split as SplitName
    : splitFor(days));
  ...
  if (rest || !session) {
    return ok({
      date, split, splitLabel: SPLITS[split].label,
      restDay: true,
      session: null,
      reason: rest
