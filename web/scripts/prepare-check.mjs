/**
 * Prepare the compiled domain modules so plain Node can import them.
 *
 * `npm run check` exercises the fitness and nutrition modules directly rather
 * than through Next, which means two small fix-ups after tsc: Node's ESM
 * resolver needs explicit .js extensions on relative imports, and the output
 * directory needs its own package.json to be treated as ES modules.
 *
